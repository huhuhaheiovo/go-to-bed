...
const sleepSoundOptions = [
    {
        id: 'sleep-sounds-p1',
        name: 'Sleep Sounds P1',
        description: 'Gentle sleep sounds for peaceful rest',
        icon: '🌙',
        color: 'from-blue-400 to-blue-600',
        file: '/mp3/SleepSounds/p1.MP3'
    }
];

export default function SleepSoundsClient() {
    const {
        globalAudio,
        globalIsPlaying,
        playGlobalAudio,
    ...
    const getCurrentAudioInfo = () => {
        ...
        return sleepSoundOptions.find(option => option.file === globalAudio.file);
    };
    ...
    const playAudioDirect = (audioType) => {
        ...
        const audioOption = sleepSoundOptions.find(option => option.id === audioType);
        if (audioOption) {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-12">
                {sleepSoundOptions.map((audio) => (
                    <div
                        ...
                        className={`relative p-6 rounded-2xl cursor-pointer transition-all duration-300 transform hover:scale-105 bg-white/10 backdrop-blur-sm hover:bg-white/20 ${currentAudioInfo && currentAudioInfo.id === audio.id
                ...
                <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                    <div className="flex items-center gap-4">
                        <button
                            ...
                            className={`w-16 h-16 rounded-full flex items-center justify-center transition-all duration-300 ${globalIsPlaying
                ...
                <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 mb-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                        {playQueue.map((audioId, index) => {
                            const audio = sleepSoundOptions.find(a => a.id === audioId);
                            return (
                        ...
                        <button
                            onClick={() => {
                                if (playQueue.length > 0) {
                                    const firstAudio = sleepSoundOptions.find(a => a.id === playQueue[0]);
                                    if (firstAudio) {
