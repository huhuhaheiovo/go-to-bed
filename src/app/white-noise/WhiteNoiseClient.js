...
export default function WhiteNoiseClient() {
    ...
    const getCurrentAudioInfo = () => {
        ...
        return whiteNoiseOptions.find(option => option.file === globalAudio.file);
    };
    ...
    const playAudioDirect = (audioType) => {
        ...
        const audioOption = whiteNoiseOptions.find(option => option.id === audioType);
        if (audioOption) {
                ...
                <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 mb-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                        {playQueue.map((audioId, index) => {
                            const audio = whiteNoiseOptions.find(a => a.id === audioId);
                            return (
                        ...
                        <button
                            onClick={() => {
                                if (playQueue.length > 0) {
                                    const firstAudio = whiteNoiseOptions.find(a => a.id === playQueue[0]);
                                    if (firstAudio) {
